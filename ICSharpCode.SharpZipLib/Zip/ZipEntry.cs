	...
	public class ZipEntry : ICloneable
	{
			...
			CompressionMethod method)
		{
			...
			if (name.Length > 0xffff) {
				throw new ArgumentException("Name is too long", ICSharpCode.SharpZipLib.Name.Of(name));
			}

			if ((versionRequiredToExtract != 0) && (versionRequiredToExtract < 10)) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(versionRequiredToExtract));
			}
		...
		public long Crc {
			...
			set {
				if (((ulong)crc & 0xffffffff00000000L) != 0) {
					throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(value));
				}
				this.crc = (uint)value;
			...
			set {
				if (value == null) {
					...
					if (value.Length > 0xffff) {
						throw new System.ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(value));
					}
		...
		public string Comment {
			...
			set {
				...
				if ((value != null) && (value.Length > 0xffff)) {
					throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(value), "cannot exceed 65535");
				}
