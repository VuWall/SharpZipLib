...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class RawTaggedData : ITaggedData
	{
		...
		{
			if (data == null) {
				throw new ArgumentNullException(ICSharpCode.SharpZipLib.Name.Of(data));
			}
	...
	sealed public class ZipExtraData : IDisposable
	{
		...
		{
			if (taggedData == null) {
				throw new ArgumentNullException(ICSharpCode.SharpZipLib.Name.Of(taggedData));
			}
		...
		public void AddEntry(int headerID, byte[] fieldData)
		{
			if ((headerID > ushort.MaxValue) || (headerID < 0)) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(headerID));
			}

			int addLength = (fieldData == null) ? 0 : fieldData.Length;

			if (addLength > ushort.MaxValue) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(fieldData), "exceeds maximum length");
			}
