...
namespace ICSharpCode.SharpZipLib.Tar
{
	...
	public class TarHeader : ICloneable
	{
		...
		public string Name {
			...
			set {
				if (value == null) {
					throw new ArgumentNullException(ICSharpCode.SharpZipLib.Name.Of(value));
				}
				name = value;
			}
		}
		...
		public long Size {
			...
			set {
				if (value < 0) {
					throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(value), "Cannot be less than zero");
				}
				size = value;
			}
		}
		...
		public DateTime ModTime {
			...
			set {
				if (value < dateTime1970) {
					throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(value), "ModTime cannot be before Jan 1st 1970");
				}
		...
		{
			if (header == null) {
				throw new ArgumentNullException(ICSharpCode.SharpZipLib.Name.Of(header));
			}
		...
		{
			if (outBuffer == null) {
				throw new ArgumentNullException(ICSharpCode.SharpZipLib.Name.Of(outBuffer));
			}
		...
		static public StringBuilder ParseName(byte[] header, int offset, int length)
		{
			...
			if (offset < 0) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(offset), "Cannot be less than zero");
			}

			if (length < 0) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(length), "Cannot be less than zero");
			}

			if (offset + length > header.Length) {
				throw new ArgumentException("Exceeds header size", ICSharpCode.SharpZipLib.Name.Of(length));
			}
		...
		{
			if (toAdd == null) {
				throw new ArgumentNullException(ICSharpCode.SharpZipLib.Name.Of(toAdd));
			}
