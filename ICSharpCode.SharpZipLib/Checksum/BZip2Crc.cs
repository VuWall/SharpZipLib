	...
	public sealed class BZip2Crc : IChecksum
	{
		...
		public void Update(byte[] buffer, int offset, int count)
		{
			...
			if (offset >= buffer.Length) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(offset), "not a valid index into buffer");
			}

			if (count < 0) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(count), "cannot be less than zero");
			}

			if (offset + count > buffer.Length) {
				throw new ArgumentOutOfRangeException(ICSharpCode.SharpZipLib.Name.Of(count), "exceeds buffer size");
			}
